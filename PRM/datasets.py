...
def pascal_voc_object_categories(query: Optional[Union[int, str]] = None) -> Union[int, str, List[str]]:
    ...
    '''
    categories = ['DontCare', 'Van', 'Cyclist', 'Pedestrian', 'Car', 'Truck', 'Misc', 'Tram', 'Person']
    '''
    if query is None:
        return categories
    else:
...
class VOC_Classification(Dataset):
    ...
    def __init__(self, data_dir, dataset, split, classes, transform=None, target_transform=None, infer_only=False):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            img = Image.open(os.path.join(self.image_dir, filename + '.png')).convert('RGB')
        if self.transform:
...
def classification(
    ...
    target_transform: Optional[Callable] = None, 
    delay_resolve=True,
    infer_only=False) -> Dataset:
    """PASCAL VOC dataset.
    """

    # object_categories = pascal_voc_object_categories()
    dataset = name + str(year)
    return VOC_Classification(data_dir, dataset, split, class_names, transform, target_transform, infer_only)
